...
function calc(el) {
    ...
    if (f[index_f].power != -1) {
        ...
        result.color = 'Green';
        result.innerHTML = `
	<text style="color:DarkRed"><text style="font-size:24px">
		Power : ${(f[index_f].power * 100).toFixed(2)}%
	</text>
	<text style="color:Pink"><text style="font-size:10px">
		PB : ${(desvioByDegree(f[index_f].desvio, distance) / 0.2167).toFixed(3)} Real pb
	</text><br>
	<text style="color:Green"><text style="font-size:36px">
		AIM : ${(((desvioByDegree(f[index_f].desvio, distance) / 0.2167) * dis ) / aim ).toFixed(3)} pb
	</text><br>
	<text style="color:Red"><text style="font-size:20px">
		Pre Calip = ${(Math.floor(f[index_f].power/(1/360))*(1/360)*f[index_f].power_range).toFixed(1)}y
	</text><br>
	<text style="color:Blue"><text style="font-size:36px">
		Caliper : ${(f[index_f].power_range * f[index_f].power).toFixed(1)}y
	</text><br>
	<text style="color:Red"><text style="font-size:20px">
		Next Calip = ${(Math.ceil(f[index_f].power/(1/360))*(1/360)*f[index_f].power_range).toFixed(1)}y
	</text><br>
	<text style="color:Pink"><text style="font-size:4px">
		Power : ${(f[index_f].power * 100).toFixed(2)}%<br>
		PB : ${(f[index_f].desvio / 0.2167 * -1).toFixed(3)} Real pb<br>
	</text><br>`;
